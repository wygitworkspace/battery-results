"""
============================================================
  锂电池内部温度预测 - PINN-LSTM 完整项目 (v3 修复版)
...
class TrainablePhysics(nn.Module):
    def __init__(self):
        super().__init__()
        self.raw_Cc = nn.Parameter(torch.tensor(3.0))
        self.raw_Cs = nn.Parameter(torch.tensor(1.5))
        self.raw_Rc = nn.Parameter(torch.tensor(0.5))
        self.raw_Rs = nn.Parameter(torch.tensor(1.5))
...
def train_one_epoch(model, loader, optimizer, criterion, device, epoch, is_pinn):
    ...
    for x_t, x_s, y, phys in loader:
        ...
        if is_pinn and epoch >= PHASE1_EPOCHS:
            ...
            loss = model.physics.adaptive_loss(l_data, l_core, l_surf)
...
def train_model(model, train_ld, val_ld, device, model_name='model'):
    ...
    if is_pinn:
        optimizer = torch.optim.Adam([
            ...
            {'params': list(model.physics.parameters()), 'lr': PHYSICS_LR},
        ])
    else:
