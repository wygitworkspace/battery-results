"""
============================================================
  锂电池内部温度预测 - PINN-LSTM 完整项目 (v17)

  基于 v16 的修改内容 (v17)：
  1. 修复自适应损失函数（最关键）
     - 对 sigma 设置下限 clamp(min=0.1)，防止权重爆炸
     - 正则项从 log_sigma 改为 torch.log(1 + s²)，保证损失函数正定（非负）
  2. 渐进式引入物理约束（Physics Warmup）
     - 新增 PHYSICS_WARMUP_EPOCHS=20 线性 warmup，避免 Phase2 突变
     - 新增 USE_ADAPTIVE_LOSS 开关（True=自适应，False=固定权重消融实验）
     - 新增 PHYSICS_LAMBDA=0.1 固定权重模式下的物理损失系数
  3. 修复 softplus + clamp 冲突
     - 去掉属性中的 softplus，直接用 clamp 约束物理范围
     - 移除训练循环中冗余的 raw_Xx.clamp_() 代码块
  4. 增加训练充分性
     - EPOCHS: 200 → 300
     - EARLY_STOP_PATIENCE: 30 → 50
     - PHASE1_EPOCHS: 15 → 20
  5. 添加 SOH 依赖的物理参数
     - 新增可训练参数 k_Rc、k_Rs（热阻随老化增大系数）
     - 新增方法 get_Rc(soh)、get_Rs(soh)
     - physics_loss 接收 soh 参数并使用 SOH 感知的热阻

  v16 内容 (保持不变)：
  1. 非均匀正则加权 Rc：Rc 从 1× 提升到 3×
  2. 加大正则系数：THERMAL_REG_LAMBDA 从 0.01 → 0.02
  3. 收紧 clamp 范围：Cc[70,120], Cs[2.5,8], Rc[1.5,4], Rs[6,12]
  4. 降低热参数学习率：THERMAL_LR 从 5e-4 → 3e-4

  物理模型：Bernardi 产热 + 二节点集总热模型
  可训练参数：Cc, Cs, Rc, Rs, k_Rc, k_Rs, OCV多项式, dOCV/dT多项式
...
# ★ v17: 物理约束渐进式引入配置
PHYSICS_WARMUP_EPOCHS = 20   # Phase2 开始后线性 warmup 的 epoch 数
USE_ADAPTIVE_LOSS = True     # True=自适应损失；False=固定权重（消融实验）
PHYSICS_LAMBDA = 0.1         # 固定权重模式下的物理损失系数
...
THERMAL_LR = 3e-4             # ★ v16: 从 5e-4 降低到 3e-4
THERMAL_REG_LAMBDA = 0.02     # ★ v16: 从 0.01 增大到 0.02
...
class TrainablePhysics(nn.Module):
    def __init__(self):
        ...
        # ★ v17: SOH 修正系数，热阻随老化（SOH↓）增大
        self.k_Rc = nn.Parameter(torch.tensor(0.0))  # Rc 随老化增大系数
        self.k_Rs = nn.Parameter(torch.tensor(0.0))  # Rs 随老化增大系数
    ...
    @property
    def Cc(self):
        return self.raw_Cc.clamp(min=70.0, max=120.0)

    @property
    def Cs(self):
        return self.raw_Cs.clamp(min=2.5, max=8.0)

    @property
    def Rc(self):
        return self.raw_Rc.clamp(min=1.5, max=4.0)

    @property
    def Rs(self):
        return self.raw_Rs.clamp(min=6.0, max=12.0)

    # ★ v17: SOH 感知热阻方法
    def get_Rc(self, soh):
        """核心→表面热阻，随 SOH 下降（老化）而增大"""
        base = self.raw_Rc.clamp(min=1.5, max=4.0)
        return base * (1.0 + self.k_Rc.clamp(min=0.0, max=2.0) * (1.0 - soh))

    def get_Rs(self, soh):
        """表面→环境热阻，随 SOH 下降（老化）而增大"""
        base = self.raw_Rs.clamp(min=6.0, max=12.0)
        return base * (1.0 + self.k_Rs.clamp(min=0.0, max=2.0) * (1.0 - soh))
    ...
    def physics_loss(self, voltage, current, soc, tc, ts, dtc_dt, dts_dt, soh=None):
        q_gen = self.compute_qgen(voltage, current, soc, tc, ts)
        # ★ v17: 使用 SOH 感知热阻（若传入 soh），否则退化为基准值
        if soh is not None:
            Rc = self.get_Rc(soh)
            Rs = self.get_Rs(soh)
        else:
    ...
    def adaptive_loss(self, l_data, l_core, l_surf):
        # ★ v17: clamp sigma 下限为 0.1，防止权重爆炸；
        #        正则项改为 log(1+s²)，保证损失函数正定（非负）
        s_d = torch.exp(self.log_sigma_data).clamp(min=0.1)
        s_c = torch.exp(self.log_sigma_core).clamp(min=0.1)
        s_s = torch.exp(self.log_sigma_surf).clamp(min=0.1)
        return (l_data / (2*s_d**2) + torch.log(1 + s_d**2) +
                l_core / (2*s_c**2) + torch.log(1 + s_c**2) +
                l_surf / (2*s_s**2) + torch.log(1 + s_s**2))

    def thermal_regularization(self, lambda_reg=THERMAL_REG_LAMBDA):
        """v16: 非均匀正则，Cs×5, Rc×3, Rs×3，全面加强约束"""
        reg = (
            (self.Cc - TARGET_Cc) ** 2 / (TARGET_Cc ** 2) +
            5.0 * (self.Cs - TARGET_Cs) ** 2 / (TARGET_Cs ** 2) +   # Cs 权重×5
            3.0 * (self.Rc - TARGET_Rc) ** 2 / (TARGET_Rc ** 2) +   # ★ v16: Rc 权重×3（从1×提升）
            3.0 * (self.Rs - TARGET_Rs) ** 2 / (TARGET_Rs ** 2)      # Rs 权重×3
        )
...
def train_one_epoch(model, loader, optimizer, criterion, device, epoch, is_pinn):
    ...
    # ★ v17: 物理损失线性 warmup（从 Phase2 起经 PHYSICS_WARMUP_EPOCHS 升到 1.0）
    if is_pinn and epoch >= PHASE1_EPOCHS:
        physics_weight = min(1.0, (epoch - PHASE1_EPOCHS) / max(1, PHYSICS_WARMUP_EPOCHS))
    else:
        physics_weight = 0.0
    for x_t, x_s, y, phys in loader:
        ...
        if is_pinn and epoch >= PHASE1_EPOCHS:
            ...
            soh = phys[:,3]  # ★ v17: 传入 soh 给 physics_loss
            l_core, l_surf = model.physics.physics_loss(V, I, soc, tc, ts, dtc, dts, soh)
            thermal_reg = model.physics.thermal_regularization()
            if USE_ADAPTIVE_LOSS:
                # ★ v17: 自适应损失（已修复正定性），physics_weight 仅作用于物理项
                # 将 l_core/l_surf 乘以 warmup 权重再传入，避免 l_data 被重复计算
                loss = model.physics.adaptive_loss(
                    l_data,
                    physics_weight * l_core,
                    physics_weight * l_surf) + thermal_reg
            else:
                # 固定权重模式（消融实验）
                loss = l_data + physics_weight * PHYSICS_LAMBDA * (l_core + l_surf) + thermal_reg
...
def train_model(model, train_ld, val_ld, device, model_name='model'):
    ...
    if is_pinn:
        thermal_params = [model.physics.raw_Cc, model.physics.raw_Cs,
                          ...
                          model.physics.k_Rc, model.physics.k_Rs]
