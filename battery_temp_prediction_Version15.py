"""
============================================================
  锂电池内部温度预测 - PINN-LSTM 完整项目 (v15)

  基于 v14 的修改内容 (v15)：
  1. 加大正则系数 10 倍：THERMAL_REG_LAMBDA 从 0.001 → 0.01
  2. 非均匀正则：Cs 权重×5，Rs 权重×3，加强对偏离最大的参数的约束
  3. 收紧 clamp 范围：Cc[50,150], Cs[2,10], Rc[1,5], Rs[5,15]
  4. 降低热参数学习率：THERMAL_LR 从 1e-3 → 5e-4
  5. 延长 Phase1 到 15 epochs，让数据拟合更充分后再引入物理约束
...
PHASE1_EPOCHS = 15            # ★ v15: 从10延长到15，让数据拟合更充分后再引入物理约束
SEED = 42
...
THERMAL_LR = 5e-4             # ★ v15: 从 1e-3 降低到 5e-4，减缓热参数漂移
THERMAL_REG_LAMBDA = 0.01     # ★ v15: 从 0.001 增大到 0.01（10倍），加强约束
...
def train_model(model, train_ld, val_ld, device, model_name='model'):
    ...
    for ep in range(EPOCHS):
        ...
        # ★ v15: 收紧热参数 clamp 范围（更强的软约束）
        if is_pinn:
            with torch.no_grad():
                model.physics.raw_Cc.clamp_(min=50.0, max=150.0)
                model.physics.raw_Cs.clamp_(min=2.0, max=10.0)
                model.physics.raw_Rc.clamp_(min=1.0, max=5.0)
                model.physics.raw_Rs.clamp_(min=5.0, max=15.0)
