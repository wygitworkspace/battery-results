"""
============================================================
  锂电池内部温度预测 - PINN-LSTM 完整项目 (v16)

  基于 v15 的修改内容 (v16)：
  1. 非均匀正则加权 Rc：Rc 从 1× 提升到 3×，解决 Rc 下降过多的问题
  2. 加大正则系数：THERMAL_REG_LAMBDA 从 0.01 → 0.02
  3. 进一步收紧 clamp 范围：Cc[70,120], Cs[2.5,8], Rc[1.5,4], Rs[6,12]
  4. 降低热参数学习率：THERMAL_LR 从 5e-4 → 3e-4
  5. Phase1 保持 15 epochs

  v15 内容 (保持不变)：
  1. 非均匀正则（Cs×5, Rs×3）
  2. 延长 Phase1 到 15 epochs

  v14 内容 (保持不变)：
  1. 热参数初始值设为目标值：Cc=87.7, Cs=4.15, Rc=2.51, Rs=8.47
  2. 添加热参数正则化损失，将参数拉向目标值（归一化 L2）
  3. 物理参数学习率从 1e-2 降低至 5e-3
  4. Phase2 总损失加入热参数正则项

  v13 原有修复内容：
  1. Phase1期间禁用早停，确保物理约束能生效
  2. 加入L2正则化防止Phase1过拟合
  3. 增大总epoch和早停耐心，给Phase2充分时间
  4. Phase2开始时重置早停计数器和学习率调度器
...
def train_model(model, train_ld, val_ld, device, model_name='model'):
    ...
    for ep in range(EPOCHS):
        ...
        if is_pinn:
            with torch.no_grad():
                model.physics.raw_Cc.clamp_(min=70.0, max=120.0)   # v15:[50,150] → v16:[70,120]
                model.physics.raw_Cs.clamp_(min=2.5, max=8.0)      # v15:[2,10] → v16:[2.5,8]
                model.physics.raw_Rc.clamp_(min=1.5, max=4.0)      # v15:[1,5] → v16:[1.5,4]
                model.physics.raw_Rs.clamp_(min=6.0, max=12.0)     # v15:[5,15] → v16:[6,12]
